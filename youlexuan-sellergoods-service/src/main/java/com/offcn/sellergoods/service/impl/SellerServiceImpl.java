...
public class SellerServiceImpl implements SellerService {
	...
	public Result updateSellerStatus(String sellerId, String status) {
		...
		//根据sellerId（主键） 修改status     ；；  update tb_seller set status = ? where sellerId = ?

		TbSeller seller = new TbSeller();// 除了sellerid和status之外，其他字段全是null
		seller.setSellerId(sellerId);
		seller.setStatus(status);

//		sellerMapper.updateByPrimaryKey(seller);  // 修改status，其他字段全部为null
		int i = sellerMapper.updateByPrimaryKeySelective(seller);//只会修改status字段，其他字段不变
